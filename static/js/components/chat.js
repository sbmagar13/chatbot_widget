...
function setBotResponse(response) {
    ...
    setTimeout(() => {
        ...
        if (response.length < 1) {
            ...
            const BotResponse = `<img class="botAvatar" src="./static/img/assistant_avatar.png"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
            ...
            for (let i = 0; i < response.length; i += 1) {
                ...
                if (Object.hasOwnProperty.call(response[i], "text")) {
                    if (response[i].text != null) {
                        const BotResponse = `<img class="botAvatar" src="./static/img/assistant_avatar.png"/><p class="botMsg">${response[i].text.replace(/(?:\r\n|\r|\n)/g, '<br>')}</p><div class="clearfix"></div>`;
                        $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                    }
                }
